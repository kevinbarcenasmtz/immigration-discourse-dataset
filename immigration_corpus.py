...
def _check_aws_credentials():
    ...
    if not os.getenv("AWS_ACCESS_KEY_ID") or not os.getenv("AWS_SECRET_ACCESS_KEY"):
        ...
        print("\nPlease run: source setup_aws.sh")
...
def load_data(
    ...
    for i in files:  # type: ignore
        ...
        if use_cache and not force_reload and cache_key in _CACHE:
            ...
            print(
                f"Cached articles_{i:03d}.jsonl ({len(_CACHE[cache_key]):,} articles)"
            )
            continue
...
def search_term(
    df: pd.DataFrame, term: str, case_sensitive: bool = False
) -> pd.DataFrame:
    """
    ...
    mask = (
        df["text"]
        .fillna("")
        .str.contains(term, case=case_sensitive, regex=True, na=False)
    )
    return df[mask]
...
def get_term_counts(df: pd.DataFrame, terms: List[str]) -> Dict[str, Dict]:
    ...
    results = {}
    for term in terms:
        mask = df["text"].fillna("").str.lower().str.contains(term.lower(), na=False)
        results[term] = {
            "count": mask.sum(),
...
def filter_by_date(
    df: pd.DataFrame, start_date: Optional[str] = None, end_date: Optional[str] = None
) -> pd.DataFrame:
    """
...
def export_to_json(df: pd.DataFrame, filename: str, format: str = "jsonl"):
    """
